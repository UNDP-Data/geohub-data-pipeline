...
def should_reproject(src_srs: osr.SpatialReference = None, dst_srs: osr.SpatialReference = None):
    ...
    if dst_srs.GetAuthorityCode(None) == 3857:return False
    try:
...
def tippecanoe(tippecanoe_cmd:str =None, timeout_secs=None):
    """
...
def dataset2fgb(fgb_dir: str = None,
                ...
                dst_prj_epsg: int = 4326, timeout_secs=None):
    """
    ...
    for lname in layers:
        try:
            ...
            # no need to remove becuase the tempdir will handle that
            #upload error blob
            ...
            with io.StringIO() as m:
                ...
                logger.error(f'Failed to convert {lname} from {src_path} to FlatGeobuf. {error_message}')
            # no need to remove becuase the tempdir will handle that
            #upload error blob
    return converted_layers

def fgb2pmtiles(fgb_layers=None, pmtiles_file_name=None, timeout_secs=None):
    ...
    if pmtiles_file_name is None:
        for layer_name, fgb_layer_path in fgb_layers.items():
            try:
                ...
                layer_timeout_secs = timeout_secs // len(fgb_layers) # split the timeout evenly for every layer
                ...
                tippecanoe(tippecanoe_cmd=tippecanoe_cmd,timeout_secs=layer_timeout_secs)
                ...
                logger.error(f'failed to convert {layer_name} from {fgb_layer_path} to PMtiles in {layer_timeout_secs} seconds.')
                #upload error file
        ...
        try:
            ...
            tippecanoe(tippecanoe_cmd=tippecanoe_cmd, timeout_secs=timeout_secs)
            with open(pmtiles_path, 'r+b') as f:
                reader = Reader(MmapSource(f))
                mdict = reader.metadata()
            assert len(fgb_layers) ==  len([vl["id"] for vl in mdict["vector_layers"]]), f'{layer_name} is not present in {layer_pmtiles_path} PMTiles file.'
            logger.info(f'Created multilayer PMtiles file {pmtiles_path}')
        except subprocess.TimeoutExpired as te:
            logger.error(f'Failed to convert {",".join(fgb_layers)} from {fgb_dir} to PMtiles in {timeout_secs} seconds.')
...
def dataset2pmtiles(src_ds=None, layers=None, pmtiles_file_name=None, timeout_secs=None):


    with tempfile.TemporaryDirectory() as temp_dir:
        fgb_layers = dataset2fgb(fgb_dir=temp_dir, src_ds=src_ds, layers=layers, timeout_secs=timeout_secs)
        if fgb_layers:
            fgb2pmtiles(fgb_layers=fgb_layers, pmtiles_file_name=pmtiles_file_name, timeout_secs=timeout_secs)
...
def dataset2cog(src_ds:gdal.Dataset=None, bands:typing.List[int]=None,  timeout_secs:int=None):
    """
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            bword = 'multiband' if not band else f'band {band}'
        ...
        logger.error(f'Failed to convert {bword} from {src_path} to COG in {timeout_secs} seconds')
        # upload error blob
    except Exception as e:
        with io.StringIO() as m:
            print_exc(
            ...
            error_message = m.getvalue()
            logger.error(f'Failed to convert {src_path} to COG. {error_message}')
...
def process_geo_file(vsiaz_blob_path: str = None, join_vector_tiles=None, timeout_secs=None):
    assert timeout_secs is not None, f'Timeout was not provided for processing {vsiaz_blob_path}'
    ...
    try:
        ...
        if vdataset is not None:
            ...
            if not join_vector_tiles:
                for layer_name in layer_names:
                    ...
                    dataset2pmtiles(src_ds=vdataset, layers=[layer_name],timeout_secs=timeout_secs)
            else:
                ...
                dataset2pmtiles(src_ds=vdataset, layers=layer_names, pmtiles_file_name=fname, timeout_secs=timeout_secs)
        ...
        for sdb in rdataset.GetSubDatasets():
            ...
            if subds_no_colorinterp_bands >= 3 or subds_photometric is not None:
                ...
                dataset2cog(src_ds=subds,timeout_secs=timeout_secs)
            else:
                for band_no in subds_bands:
                    logger.info(f'Ingesting band {band_no} from {subdataset_path}')
                    subds_timeout_secs = timeout_secs // len(subds_bands)
                    dataset2cog(src_ds=subds, bands=[band_no],  timeout_secs=subds_timeout_secs)
        ...
        if nraster_bands:  # raster data is located at root
            ...
            if max(colorinterp) >= 3 or photometric is not None:
                ...
                dataset2cog(src_ds=rdataset,  timeout_secs=timeout_secs)
                ...
                for band_no in bands:
                    ...
                    band_timeout_secs = timeout_secs // nraster_bands
                    dataset2cog(src_ds=rdataset, bands=[band_no],  timeout_secs=band_timeout_secs)
